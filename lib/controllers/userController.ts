    ...
    public async get_user(req: Request, res: Response) {
        try{
            if (req.params.id) {
                ...
                // Fetch user
                const user_data1 = await this.user_service.populateUserPosts(user_filter);
                const user_data2 = await this.user_service.populateUserReviews(user_filter);
                // Send success response
                return res.status(200).json({ data: user_data1, message: 'Successful'});
            } else {
                return res.status(400).json({ error: 'Missing fields' });
            }
        }catch(error){
